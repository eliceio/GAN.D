...
class AE(object):
    ...
    def model(self, reuse = False):
        with tf.variable_scope("autoencoder"):
            ...
            conv1 = tf.contrib.layers.conv2d(inputs=self.img, num_outputs=32, kernel_size=4, stride=2, padding="SAME", \
            ...
            conv_trans1 = tf.reshape(conv_trans1, shape=[self.batch_size, 8, 8, 128])
            ...
            conv_trans2 = tf.reshape(conv_trans2, shape=[self.batch_size, 16, 16, 64])
            ...
            conv_trans3 = tf.reshape(conv_trans3, shape=[self.batch_size, 32, 32, 32])
            ...
            conv_trans4 = tf.reshape(conv_trans4, shape=[self.batch_size, 64, 64, 16])
